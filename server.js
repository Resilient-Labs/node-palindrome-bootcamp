...
const server = http.createServer(function(req, res){
    ...
    }else if (page == '/img/poly.jpeg') {
        fs.readFile('img/poly.jpeg', function (err, data) {
                ...
                const checker = {
                    ...
                    //this converts the string into an array of letters, reverses them, and then joins them back together as a reversed string. It also makes it all lowercase.
                        if (reversed === lowerCase)  {
                            this.result = `${this.string} is a palindrome, Big Dog ,, SMILE`
                            this.img= 'img/yes.jpeg'    
                        }
                        else{
                            this.result = `${this.string} isn't a palindrome, sorry little KITTY TRY AGAIN`
                            this.img = 'img/no.jpeg'
                        }
                    }
                }
    ...
    } else if (page == '/img/no.jpeg') {
        fs.readFile('img/no.jpeg', function (err, data) {
            res.writeHead(200), { 'Content-Type': 'image/jpeg' }
            res.write(data)
            res.end()
        })
    } else if (page == '/img/yes.jpeg') {
        fs.readFile('img/yes.jpeg', function (err, data) {
            res.writeHead(200), { 'Content-Type': 'image/jpeg' }
            res.write(data)
            res.end()
        })
