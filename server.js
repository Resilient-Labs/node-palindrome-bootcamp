...
const server = http.createServer((req, res) => { //creates the http server
  const page = url.parse(req.url).pathname;   //grabs the pathname
  ...
  if (page == '/') {  //if the request is the index page
    ...
    if('result' in params){ //if result is a queried parameter in the url
      let text = params['result']
      console.log(text)
      //------------------------algorithm
      let reverse = [...text].reverse().join('')  //text is not available
      console.log(reverse)

      //let randomCoinFlip = Math.ceil(Math.random() * 2 ) === 1 ? 'heads' : 'tails'   //gives you 1 or 2

      //------------------------algorithm
      if(params['result']== reverse){   //if heads is the guess
