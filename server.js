    ...
    fs.readFile('index.html', function ( err, data) {
        ...
        if('palindrome' in params){
          ...
          if ( word.toLowerCase() === wordPal) {
            conclusion = `${word} is a Palindrome`
          } else {
      ...
      }else if (page == '/img/spiderman.jpeg') {
          fs.readFile('img/spiderman.jpeg', function (err, data) {
              res.writeHead(200), { 'Content-Type': 'image/jpeg' }
              res.write(data)
              res.end()
          })
      }else if (page == '/img/webs.jpeg') {
          fs.readFile('img/webs.jpeg', function (err, data) {
              res.writeHead(200), { 'Content-Type': 'image/jpeg' }
              res.write(data)
              res.end()
          })
